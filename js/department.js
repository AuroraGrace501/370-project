...
const button_changer = (data) => {
  ...
  button_to_inactive.innerHTML = `<button id="btn${active}" class="department-btn">${text}</button>`;

  active = data.id;
  let button_to_active = document.getElementById(active);
  text = button_to_active.textContent.trim();
  button_to_active.innerHTML = `<button id="btn${active}" class="department-btn active-btn">${text}</button>`;
    ...
    if (department == "All" || department == item.specialization) {
      ...
      <div id="container">
        ...
        <div class="doctor-details">
          ...
          <span id="doc${id}" class="hint-star star"> </span>
     ...
     `;
      ...
      for (let i = 1; i <= 5; i++) {
        if (i <= Math.floor(item.rating)) {
          star_container.innerHTML += `<i class="fa fa-star" aria-hidden="true"></i>`;
        } else if (i <= Math.ceil(item.rating)) {
          star_container.innerHTML += `<i class="fa fa-star-half-o" aria-hidden="true"></i>`;
        } else
          star_container.innerHTML += `<i class="fa fa-star-o" aria-hidden="true"></i>`;
      }
    }
  });
